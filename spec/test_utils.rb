...
module Test_utils
  ...
  def init_race
    ...
    (0..99).each do |r|
      year = Random.new.rand(2005..2016)
  ...
  def init_complete_race
    ...
    (0..[racers.length, 5].min).each do |r|
    ...
    entrants.each do |e|
      ...
      total_time += Random.new.rand(10).to_f
  ...
  RACE_FIELDS = { name: 'Oakland Triathlon', date: Date.new(2016, 8, 1),
                  lat: 37.8, lon: -122.2, city: 'Oakland', state: 'CA' }.freeze
  RACE_LOC = { type: 'Point', coordinates: [RACE_FIELDS[:lon], RACE_FIELDS[:lat]] }.freeze
  RACE_ADDR = { city: RACE_FIELDS[:city], state: RACE_FIELDS[:state], loc: RACE_LOC }.freeze
  RACER_FIELDS = { lat: 39.2, lon: -76.61, city: 'Baltimore', state: 'MD',
                   fname: 'Edgar', lname: 'Poe', byear: 1990, gender: 'M' }.freeze
  RACER_LOC = { type: 'Point', coordinates: [RACER_FIELDS[:lon], RACER_FIELDS[:lat]] }.freeze
  RACER_ADDR = { city: RACER_FIELDS[:city], state: RACER_FIELDS[:state], loc: RACER_LOC }.freeze
  EVENT_NAMES = %w(swim t1 run t2 bike).freeze
  EVENT_DIST = [1, nil, 10, nil, 25].freeze
  EVENT_UNITS = ['miles', nil, 'miles', nil, 'kilometers'].freeze
  RESULT_SECS = [60.13, 1600].freeze
  ...
  def setup_data_for_testing
    ...
    (0..EVENT_NAMES.count - 1).each do |i|
