...
feature 'Module #3 Web Race/Racer Resource Access', type: :routing do
  ...
  context 'rq01' do
    ...
    scenario 'Racer must have expected controller and routes' do
      ...
      expect(get: racer_path(test_racer_id)).to route_to(controller: 'racers', action: 'show', id: test_racer_id.to_s)
    end
  ...
  context 'rq02' do
    ...
    scenario 'Race must have expected controller and routes' do
      ...
      expect(get: race_path(test_race_id)).to route_to(controller: 'races', action: 'show', id: test_race_id.to_s)
    end
  ...
  context 'rq03' do
    ...
    scenario 'Race show page must have table of all entrants information' do
      ...
      page.all('tr').each do |row|
        cells = row.all('td')
        next unless !cells.nil? && cells.count > 0
    ...
    scenario 'Race show page has links to all racers in entrants data' do
      ...
      visit(race_path(r_id))
      expect(page.status_code).to eq(200)
      page.all('a').each do |l|
        link_uri = URI.parse(l[:href]).to_s
        next unless link_uri.split('/')[1] == 'racers'
        cur_racer_id = link_uri.split('/')[2]
  ...
  context 'rq04' do
    ...
    scenario 'Race show page has links to all racers in entrants data' do
      ...
      page.all('a').each do |l|
        link_uri = URI.parse(l[:href]).to_s
        next unless link_uri.split('/')[1] == 'races'
        cur_race_id = link_uri.split('/')[2]
